/*
  ==========================================================
  Power Station Control Center — ESP32 POLE Node (P001)
  ==========================================================
  Flash this onto the ESP32 sitting on the POLE side of the bench
  rig. For the HOME side, flash firmware/esp32_home_node instead —
  ...
  ==========================================================
  CIRCUIT DIAGRAM — POLE NODE (P001)
  ==========================================================
 ...
 POLE 12V(+)o-------> IP+           IP- --+-> POLE LINE    |  VCC(+) and GND    |
  ...
  All GND nets above (12V supply return, ACS712, ESP32, LEDs, USB
  source #1) are one single common ground.

  --------------------------------------------------------------
  DEMO TIP: flip the FAULT SWITCH to cut power to the HOME unit only.
  This pole keeps reporting ON (its own lamp + current stay live)
  while Home reports OFF — that's exactly the "LOCAL FAULT" case the
  backend's outage engine (engines/outage.py) is built to detect and
  auto-ticket. Pull the WiFi/router instead to see the BLUE LED start
  blinking and the dashboard mark this pole "NOT SENDING DATA" once
  its 2-minute stale window passes.
...
// ---------------- DEVICE IDENTITY (fixed — this is the pole board) ----------------
const char* DEVICE_ID    = "P001";      // Must match a row in gateway/auth.py
const char* DEVICE_TYPE  = "pole";
const char* DEVICE_TOKEN = "token123";  // Must match DEVICE_ID's row in gateway/auth.py
// --------------------------------------------------------------------
...
// Measure OUT with a multimeter at zero current (before wiring up the load)
// and set this precisely — don't auto-calibrate at boot: the pole's lamp
// load is always-on, so a "zero current at startup" reading would be wrong.
const float ACS712_MIDPOINT_MV = 2500.0;
// --------------------------------------------------------------------
